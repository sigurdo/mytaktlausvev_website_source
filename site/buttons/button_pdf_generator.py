...
def button_pdf_generator(
    ...
    button_full_width_px = mm_to_px(button_visible_width_mm + 2 * button_backside_padding_mm)
    button_full_height_px = mm_to_px(button_visible_height_mm + 2 * button_backside_padding_mm)
    ...
    num_buttons_horizontal = (page_width_px - page_margin_left_px - page_margin_right_px - button_minimum_distance_px) // (button_full_width_px +  button_minimum_distance_px)
    num_buttons_vertical = (
        page_height_px - page_margin_top_px - page_margin_bottom_px - button_minimum_distance_px
    ...
    array_left_px = (
        page_width_px - (num_buttons_horizontal * button_full_width_px) - ((num_buttons_horizontal - 1) * button_minimum_distance_px)
    ) // 2
    array_top_px = (
        page_height_px - (num_buttons_vertical * button_full_height_px) - ((num_buttons_vertical - 1) * button_minimum_distance_px)
    ) // 2
        ...
        def paint_page():
            for r in range(num_buttons_vertical):
                for c in range(num_buttons_horizontal):
                    ...
                    offset_left = array_left_px + (c * (button_full_width_px + button_minimum_distance_px))
                    offset_top = array_top_px + (r * (button_full_height_px + button_minimum_distance_px))
